        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 mb-20">
          ...
          <div>
            ...
            <ul className="space-y-4">
              {['Check & Savings', 'Business Accounts', 'Credit Card', 'Investments', 'Loans'].map((item) => (
                <li key={item}>
                  <button 
                    onClick={() => {
                      if (item === 'Credit Card') onNavigate('credit-card');
                      if (item === 'Check & Savings') onNavigate('check-savings');
                    }}
                    className="text-sm font-semibold text-gray-500 hover:text-[#064E3B] transition-colors"
                  >
                    {item}
                  </button>
                </li>
          ...
          <div>
            ...
            <ul className="space-y-4">
              {['About Us', 'Careers', 'Security', 'Press', 'Legal'].map((item) => (
                <li key={item}>
                  <button 
                    onClick={() => {
                      if (item === 'About Us') onNavigate('about');
                      if (item === 'Security') onNavigate('security');
                    }}
                    className="text-sm font-semibold text-gray-500 hover:text-[#064E3B] transition-colors"
                  >
                    {item}
          ...
          <div>
            ...
            <ul className="space-y-4">
              {['Help Center', 'FAQs', 'Contact Support', 'Privacy Policy'].map((item) => (
                <li key={item}>
                  <button 
                    onClick={() => {
                      if (item === 'Help Center' || item === 'FAQs') onNavigate('help');
                    }}
                    className="text-sm font-semibold text-gray-500 hover:text-[#064E3B] transition-colors text-left"
                  >
                    {item}
                  </button>
                </li>
