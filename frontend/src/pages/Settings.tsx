...
export const Settings: React.FC = () => {
  ...
  const toggleNotification = (type: string, channel: 'email' | 'push') => {
    setNotificationSettings(prev => ({
      ...prev,
      [type]: {
        ...prev[type as keyof typeof notificationSettings],
        [channel]: !prev[type as keyof typeof notificationSettings][channel]
      }
    }));
  };
                  ...
                  <div className="bg-white p-10 rounded-[3rem] border border-gray-50 shadow-sm space-y-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-2">
                        ...
                        <input
                          defaultValue={user?.first_name || 'Alex'}
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          <input
                            defaultValue={user?.email || 'alex.rivera@example.com'}
                ...
                >
                  <div className="bg-white p-10 rounded-[3rem] border border-gray-50 shadow-sm space-y-10">
                    ...
                    <div className="space-y-8">
                      {[
                        { key: 'security', title: 'Security Alerts', desc: 'Get notified of unusual login attempts or suspicious activity.' },
                        { key: 'transactions', title: 'Transaction Updates', desc: 'Receive alerts for every purchase and deposit made.' },
                        { key: 'reminders', title: 'Payment Reminders', desc: 'Stay on top of upcoming bill payments and transfers.' },
                        { key: 'features', title: 'New Features', desc: 'Be the first to hear about product updates and new tools.' },
                      ].map((item) => (
                        <div key={item.key} className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-6 rounded-[2rem] border border-gray-50 hover:bg-gray-50 transition-all group">
                          ...
                          <div className="flex gap-8">
                            <div className="flex flex-col items-center gap-2">
                              ...
                              <button
                                ...
                                className={`w-12 h-6 rounded-full relative transition-colors ${notificationSettings[item.key as keyof typeof notificationSettings].email ? 'bg-[#064E3B]' : 'bg-gray-200'}`}
                              >
                                <div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-all ${notificationSettings[item.key as keyof typeof notificationSettings].email ? 'left-6.5' : 'left-0.5'} shadow-sm`} />
                              </button>
                            </div>
                            <div className="flex flex-col items-center gap-2">
                              ...
                              <button
                                ...
                                className={`w-12 h-6 rounded-full relative transition-colors ${notificationSettings[item.key as keyof typeof notificationSettings].push ? 'bg-[#064E3B]' : 'bg-gray-200'}`}
                              >
                                <div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-all ${notificationSettings[item.key as keyof typeof notificationSettings].push ? 'left-6.5' : 'left-0.5'} shadow-sm`} />
                              </button>
                            </div>
                          </div>
                        </div>
